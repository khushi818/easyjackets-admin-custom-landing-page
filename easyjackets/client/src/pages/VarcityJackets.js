...
const VarcityJackets = () => {
  const [selectedImage, setSelectedImage] = useState(null);
  const [metaData , setMetaData] = useState({})
  
  const getMetaTags = async() =>{
    const { data } = await axios.get(`/api/v1/metadata/varsity`)
    ...
    <Layout title={metaData?.title} description={metaData?.description} keywords={metaData?.keywords}>
      <div className="mt-5">
        <h1 className="text-uppercase text-center">
          <span className="text-danger text-uppercase">Fabric </span>Leather
        </h1>
